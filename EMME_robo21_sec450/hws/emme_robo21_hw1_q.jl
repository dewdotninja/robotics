...
We want to generate a 3-segment trajectory as follows:

* Segment 1: at $t=0$ from $q(0) = 0$, $\dot{q} = 0$ to $t = 2, q(2) = 10, \dot{q}(2) = 10$
* Segment 2 : continue from segment 1 with  $q(2) = 10, \dot{q}(2) = 10$ and end with $t = 4, q(4) = -5, \dot{q}(4) = -5$
* Segment 3 : continue from segment 2 with $q(4) = -5, \dot{q}(4) = -5$ and end with $t = 5, q(5) = 0, \dot{q}(5) = 0$

Create time vector t1 for segment 1,2 (period =  2 second) and t2 for segment 3 (period = 1 second)
"""
...
md"""
Use qpoly() function to generate output vector for each of the segment 
"""
...
md"""
Join the segments to become the whole trajectory
"""
...
md"""
Plot the trajectory as follows
"""
